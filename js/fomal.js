...
$.ajax({
  ...
  data: {
    key: '7ESBZ-UZ3WZ-NHXXD-7GH72-2T3SJ-VFBNU',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(118.78, 32.07, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function winResize() {
  try {
    ...
    if (offsetWid <= 768) {
      winbox.resize(offsetWid * 0.95 + "px", "35%").move("center", "center");
    } else {
      winbox.resize(offsetWid * 0.6 + "px", "25%").move("center", "center");
    }
  } catch (err) {
